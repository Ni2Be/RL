...
Action MCTS_Agent<State_T>::select_action()
{
	...
	for (auto child : root->children)
		std::cout << "value: " << child->value << ", visits: " << child->visits << "\n";
	std::cout << "\n";
	//ENDDEBUG
...
template <class State_T>
MC_Node<State_T>* MC_Node<State_T>::uct() const
{
	if (children.size() <= 0)
	{
	...
	float best_path_uct = 0.0f;
	for (auto child : children)
	{
		float child_visits = static_cast<float>(child->visits);
		float child_uct = (child->value / child_visits) + (m_c * std::sqrt(std::log(this->visits) / child_visits));
...
{
	//TODO backpropagate the childs value weighted
